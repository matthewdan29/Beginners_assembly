...
; 				  NASM, demonstrating the use of escape 
; 				  sequences to do simple "full-screen" text output. 
; 
; 
; Bulid using these commands Linux: 
; 	nasm -f elf -g -F stabs eatterm.asm
; 	ld -o eatterm eatterm. 
; 
; Build using these commands on Windows: 
; 	nasm -f win32 eatterm.asm -o eatterm.o
; 	ld -o eatternm eatterm.o
; 
; 
...
; mind that the code shown here willl only work up to 99 X 99. 
...
ClrScr: 
	...
	mov edx, CLEARLEN		; Pass the length of terminall control string
...
; 		passed in AL and AH and calls sys_write to postion the 
; 		console cursor to that X,Y position. Writing text to the console after calling GoToXY begin display of
; 		text at that X,Y position. 
...
;------------------------------------------------------------------------------------------------------------
; WriteCtr: 	Sent a string centered at an 80-char wide Linux console
; UPDATED: 	1/6/26
...
; 		80-column dispaly. Calculates the X for the passed-in
...
; Wait for the user to press Enter
	...
	mov ebx, 0				; Specify FIle discriptor 0: Stdin
