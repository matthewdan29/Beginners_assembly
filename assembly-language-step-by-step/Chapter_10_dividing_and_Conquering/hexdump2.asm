...
; Version				: 1.0
; Created date 				: 12/30/25
; Last update				: 12/30/25
; Author				: Matthew K. Daniels (via Jeff Duntemann)
; Description 				: A simple hex dump utility demonstrating the use of assembly lang procedures
; 
; 
...
; 
; Build on windows using these commands: 
; 	nasm -f win64 hexdump2.asm -o hexdump.o
; 	ld hexdump2 hexdump2.o
; 
...
; All printable characater "play throught" as themselves. The high 128
...
; MODIFIES: 	Nothing
; CALLS: 	DumpChar
; DESCRIPTION: 	The hex dump line string is cleared to binary 0 by calling DumpChar 16 times, passing it 0 each time.
...
; CALLS: 		Nothing
; DESCRIPTION: 		The value passed in EAX will be put in both the hex dump portion and in the ASCII portion,
; 			st the position passed in EDX, represented by a space where it is not a printable character. 


DumpChar: 
	push ebx		; Save caller's EBX
	push edi		; Save callers's EDI
...
; Look up nybble character and insert it into the string: 
...
PrintLine: 
	...
	mov ecx, DumpLin		; Pass offset of lin string
...
; 					starting in on a new buffer full of data. Caller must test value in EBP: if EBP
; 					contains zero on return, we hit EOF on stdin Less than 0 in EBP on return 
...
LoadBuff: 
	...
	mov edx, BUFFLEN		; Pass number of bytes to read at on pass
	int 80h				; Call sys_read to fill the buffer
	mov ebp, eax			; Save # of bytes read from file for later 
	xor ecx, ecx			; Clears buffer pointer ecx to 0
...
; Whatever initialization needs doing before the loop scan starts is here: 
	...
	cmp ebp, 0			; If ebp=0, sys_read readched EOF on stdin 
	jbe Exit 
