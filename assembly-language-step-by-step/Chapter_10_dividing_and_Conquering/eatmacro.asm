...
; 
; Build using these commands on window: 
; 	nasm -f win64 eatmacro -o eatmacro.o
; 	lb -o eatmacro eatmacro.o
; 
; 
...
; add additional ASCII digit encoding to hte end of Digits. Keep in 
...
SECTION .text					; Section contining code 

;-------------------------------------------------------------------------------------------------------------
; ExitProg:	Terminated program and return to Linux
; UPDATED: 	1/7/26
; IN: 		Nothing
; MODIFIES: 	Nothing
; CALLS: 	Kernel sys_exit
; DESCRIPTION: 	Calls sys_edit to terminate the program and return control to Linux
...
%marco	WriteStr 2			; %1 = String address; %2 = string length
...
; Use WriteStr macro to write control string to console: 
	...
	pop edx 		; Restor pertinent registers 
	pop ecx		
	pop ebx
	pop eax
%endmacro
...
; Poke	the Y digits: 
	...
	mov cv, word [Digits+edx*2]		; Fetch decimal digits to CX
...
%macro	WriteCtr 3			; %1 = row; %2 = String addr; %3 = String length
	...
	pop ebx				; Restor callers EBX	
%endmacro 

global _start 				; Linker needs this to find the entry point 

_start: 
	nop				; Keep no-op gdb happy

; First we clear the terminal display
	ClrScr
;Then we post the ad message centered on the 80-wide consol: 
