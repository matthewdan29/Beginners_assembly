...
; Description		: A text file I/O demo for Linux, using NASM 
; 
; Build using these commands: 
; 	nasm -f elf32 -g -F stabs textfile.asm
; 	nasm -f elf32 -g -F stabs linlib.asm
; 	gcc -m32 textfile.o linlib.o -o textfile
; 
; Not that this program requires serveral procedures 
; in an external named LINLIB.ASM. 

[SECTION .data]		; Section conatining initialized data
...
Errl db 'ERROR: The first command line argument must be an integer!',10,0
...
HELPSIZE EQU $-HelpMsg
	...
	db 'are entered, random text written to the file. The msg ',10.0 
...
HELPLEN   EQU 72	; Defin length of a line of help text data 
HelpLin   resb HELPLEN  ; Reserve space for disk-based help text line
BUFSIZE   EQU		; Define length of text line buffer buff
...
man: 
	push ebp	; Set up stack frame from debugger
	mov ebp, esp 
	push ebx	; Program must preserve EBP, EBX, ESI & EDI
	push esi 
	push edi 
;; Everything before this is boilerplate; use it for all ordinary app!
...
chkarg2: 
	...
	push IntFromt		; Push address of integer formatting code 
	...
	mov eax, Errl		; Load eax with address of error message #1
...
;; are, we concatenated them into a single string no more than BUFSIZE 
;; chars in size. (Yes, I Know this does what strncat does...)
chkdata: 
	cmp dword [ebp+8],3	; Is there s second argument?
...
getlns: mov ebx, 2		; We know we have at least arg(2), start there
...
grab: 	mov esi, [ebx+ebx*4]	; Copy pointer to next arg into esi 
...
.next:	mov byte [edi], ' '	; Coyp space to buff to separate args 
	...
	jae addnul 		; if edi = arg count, we're done 
	jmp grab 		; And go back and copy it 

addnul:	mov byte [edi],0	; Tack a null on the end of buff
...
gohome: pop edi			; Restore save registers 
	pop esi
	pop ebx
...
;-----------------------------------------------------------------------------
; Disk-based mini-help subroutine -- Last update 06/01/2026
; 
...
; cannot be opend, a very short memory-based message is displayed instead. 
;-----------------------------------------------------------------------------
...
.chkln:	cmp dword [ebx], 0	; Does help msg pointer point to a null?
	...
	ret 			; If yes, go home
.show: 	push ebx		; Push address help line on the stack 
	call printf		; Display the line 
	add esp, 4		; Clean up the stack 
	add ebx, HELPSIZE	; Increment address by lenght of help line
	jmp .chkln		; Look back and check to see if we done yet 

showerr: 	
	push eax 		; On enty, eax contains addr of error message  
	call printf 		; Show the error message 
	add esp, 4		; Clean up the stack 
	ret 			; Go home; no returned value

randline: 
	mov ebx, BUFSIZE	; BUFSIZE tells us how many chars to pull 
	mov byte [Buff+BUFSIZE+1] ; Put a null at end of buffer first 
